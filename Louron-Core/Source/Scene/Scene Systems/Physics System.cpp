...
#pragma region RigidbodyComponent
	...
	void PhysicsSystem::RemoveRigidBody(Entity entity, Scene* scene) {
		...
		scene->m_Registry.remove<RigidbodyComponent>(entity);

		L_CORE_INFO("Rigidbody Has Been Successfully Removed from Entity ({}:{}).", entity.GetName().c_str(), std::to_string(entity.GetUUID()).c_str());
	}
...
#pragma region Colliders
	...
	void PhysicsSystem::RemoveCollider(Entity entity, Scene* scene, PxGeometryType::Enum colliderType) {
		...
		if (colliderType == PxGeometryType::eSPHERE) {
			...
			collider.Release();
			scene->m_Registry.remove<SphereColliderComponent>(entity);

			L_CORE_INFO("Sphere Collider Has Been Successfully Removed from Entity ({0}:{1}).", entity.GetName(), std::to_string(entity.GetUUID()));
		}
		...
		if (colliderType == PxGeometryType::eBOX) {
			...
			collider.Release();
			scene->m_Registry.remove<BoxColliderComponent>(entity);

			L_CORE_INFO("Box Collider Has Been Successfully Removed from Entity ({0}:{1}).", entity.GetName(), std::to_string(entity.GetUUID()));
		}
	}
